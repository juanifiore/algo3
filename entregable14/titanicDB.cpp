...
void printmatriz(const vector<vector<char>>& matrix) {
...
void printmapmap(const map<int, map<int, int>>& nestedMap) {
    for (const auto& outerPair : nestedMap) {
        cout << "clave" << outerPair.first << ":\n";
...
int ford_felkurson(map<int,map<int,int>>& grafo, int s, int t){
	...
	while(hay_camino_aumento){
		...
		for(int v = t; v != s; v = padre[v]){
			...
			if(grafo.find(v) != grafo.end() and grafo[v].find(u) != grafo[v].end()){
...
int main(){
	...
	//string xyp; 
	int x,y,p;
        //while(!getline(cin, xyp).fail() && !xyp.empty()){
	while(cin >> x >> y >> p){
		t++;
                //int x,y,p;

                //istringstream iss(xyp);
                //iss >> x >> y >> p;
